...
import { createRoomSchema, sendMessageSchema, getRoomMessagesSchema, markMessagesReadSchema } from '../validation/chat.schemas.js';
...
export class ChatController {
    ...
    static async getMessages(req, res) {
        try {
            ...
            const result = await getChatMessages.exec(validatedQuery.roomId, user.id, user.userType, validatedQuery.page, validatedQuery.limit);
            if (!result.ok) {
    ...
    static async sendMessage(req, res) {
        try {
            ...
            const result = await sendMessage.exec(validatedData.roomId, user.id, user.userType, validatedData.messageType, validatedData.content, validatedData.fileUrl ? {
                fileUrl: validatedData.fileUrl,
                fileName: validatedData.fileName || '',
                fileSize: validatedData.fileSize || 0,
                mimeType: validatedData.mimeType || ''
            } : undefined, validatedData.metadata);
            if (!result.ok) {
