...
// create partial user
export async function createPartialUser(req, res) {
    ...
    try {
        ...
        console.log("Error in createPartialUser controller:", error.message);
        return res.status(500).json({ ok: false, message: 'Internal Server Error.' });
    }
}
// create full user
export async function createFullUser(req, res) {
...
export async function requestOtp(req, res) {
    ...
    try {
        ...
        const result = await uc.exec(subjectType, subjectRef, contact);
        if (!result.ok) {
        ...
        return res.status(500).json({ ok: false, message: error?.message ?? 'OTP request failed.' });
    }
}
// verify otp
export async function verifyOtp(req, res) {
    ...
    try {
        ...
        const statusMap = { invalid: 401, expired: 400, locked: 429, not_found: 404, invalid_token: 401 };
